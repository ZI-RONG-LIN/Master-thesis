...
run;
/*把行列轉置*/
proc transpose data = work.aa out=b;
by _col0 _COL1 _col2;
...
/*測值超過15.4的，over=1*/
DATA PM1;SET PM;
IF  COL1 >15.4 THEN OVER=1;
ELSE OVER=0;
RUN;
/*計算超標小時數*/
DATA PM2;SET PM1;
BY _COL0;
IF FIRST._COL0 THEN OVER_HOUR =0;
OVER_HOUR+OVER;
RUN;
/*沒有測值的，LOSS=1*/
DATA PM3;SET PM2;
...
DATA PM4;SET PM3;
BY _COL0;
IF FIRST._COL0 THEN LOSS_HOUR =0;
LOSS_HOUR+LOSS;
RUN;
/*計算日累積濃度*/
DATA PM5;SET PM4;
BY  _COL0;
IF FIRST._COL0 THEN TOTAL_CON=0;
TOTAL_CON+COL1;
RUN;
/*留下最後一筆資料*/
DATA PM6;SET PM5;
...
IF LAST._COL0;
RUN;
/*留下日期、測站、測項、超標小時值、缺值小時值、日累積濃度，其他變項篩掉*/
DATA PM7;SET PM6;
KEEP _COL0 _COL1 _COL2  OVER_HOUR  LOSS_HOUR  TOTAL_CON;
RUN;

data pm8; set pm7;
keep _COL0 _COL1 _COL2 col2 OVER_HOUR LOSS_HOUR TOTAL_CON total_hour;
run;
